...
// Small pill used for dataset metadata. `tone` maps to the Egypt ODP palette.
...
export default function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-3 py-1 text-sm font-medium sm:px-2.5 sm:py-0.5 sm:text-xs ${TONES[tone]} ${className}`}
    >
      {children}
    </span>
  )
