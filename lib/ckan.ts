...
export type DatasetCard = {
  ...
  groups: { name: string; title: string }[] // themes this dataset belongs to
  tags: string[] // free-text tags (display names)
}
...
// Department (CKAN org) and Theme (CKAN group) share the same tile shape.
export type Department = { namespace: string; title: string; count: number }
export type Theme = Department
...
// All CKAN groups (themes) with their dataset counts, sorted by count desc.
export async function getAllThemes(): Promise<Theme[]> {
  const result = await ckanAction('group_list', { all_fields: 'true' })
  return (Array.isArray(result) ? result : [])
    .map((g: any) => ({
      namespace: g.name as string,
      title: (g.title || g.name) as string,
      count: (g.package_count ?? 0) as number,
    }))
    .filter((t: Theme) => t.count > 0)
    .sort((a: Theme, b: Theme) => b.count - a.count)
}
